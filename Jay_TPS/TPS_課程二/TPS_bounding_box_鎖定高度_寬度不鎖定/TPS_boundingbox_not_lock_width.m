...
for k=1:1:length(allFile)
    ...
    x_magnify=ceil(x*x_magnification);    %放大ｘ        用ceil是怕floor座標會有0值   放大高度
    y_magnify =ceil(y*x_magnification);   %放大寬度      寬度放大倍率跟高度一樣
    %figure,plot(y_magnify,-x_magnify,'x'),axis equal,axis tight
    %%show出ｐｌｏｔ
    ...
    rtmap_300point=zeros(Bounding_box,max(y1)+1);  %製造一個空白的矩陣
     ...
     sx=[x1,y1];
     figure,plot(y1,-x1,'x'),axis equal,axis tight
     fnMat = allFile(k).name;    
     %open link file
     fpLink = fopen(['..\' 'drawline_bunding_notlockwidth' num2str(k) 'a.point' ], 'wt' );
