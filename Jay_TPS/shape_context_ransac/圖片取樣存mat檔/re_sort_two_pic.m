...
for i=2:1:size(save_pic_cut,2)
    ...
    figure,plot(regulate_pic(:,1),regulate_pic(:,2),'o'),title([num2str(cut_row),'*',num2str(cut_col)])
    hold on;
    clear x1 y1 y2 y2a
    x1=regulate_pic;
    y1=regulate_pic;
    regulate_pic(:,1)=regulate_pic(:,1)+50;
    y2=regulate_pic;
    y2a=regulate_pic;
    plot(regulate_pic(:,1),regulate_pic(:,2),'x')
end
save face_x_y x1 y1 y2 y2a;