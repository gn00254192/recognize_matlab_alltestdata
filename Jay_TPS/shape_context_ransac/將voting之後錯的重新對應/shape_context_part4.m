...
for i=1:1:size(sec_wrong,1)
    for j=1:1:size(correct,1)
        min_distan(j,i)=sqrt((sec_wrong(i,5)-correct(j,5))^2+(sec_wrong(i,6)-correct(j,6))^2);       
        %沒對到的點減去所有對到的點,求距離
    end
    plot(sec_wrong(i,5),sec_wrong(i,6),'Rx')
    plot(sec_wrong(i,7),sec_wrong(i,8),'gx')
    hold on;
end


for h=1:1:size(sec_wrong,1) 
[min_1,index]=min(min(min_distan));%找出最小距離
temp=find(min_distan==min_1);

if(mod(temp,size(min_distan,1))==0)
    sec_wrong_number=floor(temp/size(min_distan,1));
    correct_number=size(min_distan,1);
else
    sec_wrong_number=floor(temp/size(min_distan,1))+1;
    correct_number=mod(temp,size(min_distan,1));
end
...
ind=cvec(ind_good);
hold on
plot(sec_wrong(sec_wrong_number,5),sec_wrong(sec_wrong_number,6),'Rx')
hold on
plot(correct(correct_number,5),correct(correct_number,6),'go')
hold on
plot(sec_wrong(sec_wrong_number,9),sec_wrong(sec_wrong_number,10),'Rx')
hold on
plot([correct(correct_number,5) correct(correct_number,7)]',[correct(correct_number,6) correct(correct_number,8)]','k-');
hold on
plot([sec_wrong(sec_wrong_number,5) sec_wrong(sec_wrong_number,9)]',[sec_wrong(sec_wrong_number,6) sec_wrong(sec_wrong_number,10)]','k-');
hold on
r=0.10;
x0=sec_wrong(sec_wrong_number(1,1),9);
y0=sec_wrong(sec_wrong_number(1,1),10);
t=linspace(0, 2*pi, 360);
x=x0+r*cos(t);
y=y0+r*sin(t);
plot(x,y);
hold on
plot(sec_wrong(:,7),sec_wrong(:,8),'kx')
hold off

for i=1:1:size(sec_wrong,1)
        temp_dis(i,sec_wrong_number(1,1))=sqrt((sec_wrong(i,7)-sec_wrong(sec_wrong_number(1,1),9))^2+(sec_wrong(i,8)-sec_wrong(sec_wrong_number(1,1),10))^2);          
end       
    min_distan(:,sec_wrong_number)=999;
    [min_1,index]=min(min(min_distan));%找出最小距離
end

for i=1:1:size(temp_dis,1)
    for j=1:1:size(temp_dis,2)
        if(temp_dis(j,i)<0.1 && temp_dis(j,i)==min(temp_dis(:,i)))
            sec_wrong(i,12)=1; 
            sec_wrong(i,7)=sec_wrong(j,7);
            sec_wrong(i,8)=sec_wrong(j,8);
        end
    end
end
...
for i=1:1:size(sec_wrong,1)
    if(sec_wrong(i,12)==1)
        plot([sec_wrong(i,5) sec_wrong(i,7)]',[sec_wrong(i,6) sec_wrong(i,8)]','k-');
        hold on
    else
...
% ind=cvec(ind_good);
% hold on
% plot(sec_wrong(1,5),sec_wrong(1,6),'Rx')
% hold on
% plot(correct(1,5),correct(1,6),'go')
