...
plot(sec_wrong(:,7),sec_wrong(:,8),'kx'),title(['total correct point=' num2str(sum(corr_map(:,12))+size(correct_point(:,1),1)+sum(sec_wrong(:,12))) ])

temp1=0;
temp_count=1;
for i=1:1:size(corr_map,1)
    if(corr_map(i,12)==1)
        sum1=sqrt((corr_map(i,7)-corr_map(i,9))^2+(corr_map(i,8)-corr_map(i,10))^2);
        temp_array(temp_count,1)=sum1;
...
temp2=0;
for i=1:1:size(correct_point,1)
        sum2=sqrt((correct_point(i,7)-correct_point(i,9))^2+(correct_point(i,8)-correct_point(i,10))^2);
        temp_array(temp_count,1)=sum2;
        temp_count=temp_count+1;
        temp2=sum2+temp2;
end
...
for i=1:1:size(sec_wrong,1)
    if(sec_wrong(i,12)==1)
        sum3=sqrt((sec_wrong(i,7)-sec_wrong(i,9))^2+(sec_wrong(i,8)-sec_wrong(i,10))^2);
        temp_array(temp_count,1)=sum3;
        temp_count=temp_count+1;
        temp3=sum3+temp3;
    end
end
pic_dis_temp=((98-size(temp_array,1))*0.1+(temp1+temp2+temp3))/98;
pic_dis=(temp1+temp2+temp3)/(sum(corr_map(:,12))+size(correct_point(:,1),1)+sum(sec_wrong(:,12)));
