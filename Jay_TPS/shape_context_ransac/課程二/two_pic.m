...
select_point=300;
Bounding_box=0.8;
cut_row=75;  %有幾列，不是切幾刀
cut_col=75;  %有幾行
pic=imread('6325587836665856.tif');   
...
x1=x_y_merge(:,1)-xminimum+0.01;         %所有座標減去最小值,讓圖片可貼在(1,1)上
y1=x_y_merge(:,2)-yminimum+0.01;         %所有座標減去最小值,讓圖片可貼在(1,1)上
...
x_magnify=x1*x_magnification;    %放大ｘ   用ceil是怕floor座標會有0值
y_magnify =y1*y_magnification;   %放大ｙ


size(x_y_merge)
x_y_merge=[x_magnify,y_magnify];
regulate_pic(:,1)=x_y_merge(:,2)
regulate_pic(:,2)=-x_y_merge(:,1)
figure,plot(regulate_pic(:,1),regulate_pic(:,2),'o'),axis equal,axis tight

x1=regulate_pic;
y1=regulate_pic;

save data select_point Bounding_box cut_row cut_col
save temp x1 y1;
%-------------------pic2--------------------------------
clc;
clear;
load data.mat
pic2=imread('pic_part2test.tif');
...
size(x_y_merge)
x_y_merge=[x_magnify,y_magnify];
regulate_pic2(:,1)=x_y_merge(:,2)+0.2
regulate_pic2(:,2)=-x_y_merge(:,1)-0.2
figure,plot(regulate_pic2(:,1),regulate_pic2(:,2),'x'),axis equal,axis tight


y2=regulate_pic2;
y2a=regulate_pic2;
load temp.mat
save line_pic9_pic8 y2 y2a x1 y1;