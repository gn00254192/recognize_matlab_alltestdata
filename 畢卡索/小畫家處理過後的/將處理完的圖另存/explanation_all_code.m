...
for k=1:1:length(allFile)
    pic=imread(allFile(k).name);%對每一張tif圖片做處理
       ...
       for xi=1:1:length(x1)                       %x座標跟y座標所有點減去x(1)及y(1)每個算出距離後，找出最大值
            ...
            pic_progress=xi*k/(length(x1)*length(allFile))*100
       end
       maxlength(1,k)=mx;
       ...
       %open link file
       fpLink = fopen([num2str(k) 'a.point' ], 'wt' );
...
for i=1:1:length(allPoint)
    ...
    rtmap=zeros(ceil(allpicmaxlen)+200);%製造一個空白的矩陣
...
for picnum=1:1:originpic-1
    ...
    for i=1:1:size(rtmap,1)
        for j=1:1:size(rtmap,2)
            ...
            sum=Disp2o+sum;
            fft_progress=(i*j*picnum)/(size(rtmap,1)*size(rtmap,2))*(originpic-1)*100
        end
    end
